...
| 未拍板决策 | 凭证轮换存储方案（`M0-04凭证轮换存储方案.md` 已起草待评审：新增第 16 张表 app_credential + 轮换状态机，3 个确认点） |
...
`doc/开发文档/`（M0 契约已评审通过 v1.0；M0-04 凭证方案待评审）：
...
| `M0-04凭证轮换存储方案.md` | （待评审）app_credential 凭证表 + ACTIVE/ROTATING/RETIRED 状态机，解决 app 表凭证单列与设计 §1.2 轮换并存的缺口 |
...
- **M0 契约**（`src/main/resources/doc/开发文档/`）：链引擎 / 映射语义 / 客户端对账三份（已评审通过 v1.0，M2 编码依据）+ 凭证轮换存储方案 M0-04（待评审）
- `README.md` — 项目索引

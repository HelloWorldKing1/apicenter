...
- **集成测试隔离后台 worker（2026-09-12）**：`retry-worker-initial-delay-ms` / `alert-worker-initial-delay-ms` 默认 0（启动即跑一轮），手动驱动 `scan()` 的用例必须把它们与 `*-fixed-delay-ms` 一起置大（M2/M3/M4/StateChain 已置 1h）。
...
- **列表列口径（2026-09-12）**：应用列表首列为「ID」（`app` 表主键即 `app_id`，无数字 id；不要再加重复列、也不要把「ID / 应用标识」两个词混写在同一列头），凭证角标列已移除；接口列表新增数字主键「ID」列（监控/调用日志里的接口ID，普通表头不带 tooltip），「应用」列保持只显示应用名称（不要把 appId 塞进同一格）。

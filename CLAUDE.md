...
| `B1协议参数手动验收测试方案.md` | **B1 手动复验方案（v1.0，四阶段约 20 分钟，按实施后实际行为校准）**：准备（真实 USGS / WireMock 二选一）；阶段一保存期校验 8 例（均 `40001` + msg 片段）；阶段二**出站报文取证**（声明/根元素/命名空间 6 组，**含基线预期值**）；阶段三边界与解耦（ack 同源 / 入站不解包 / GET 不带 body）；阶段四即时生效·回滚·复制·界面；§5 **常见误判表**（7 条）；§6 收尾 SQL + `clean test` 回归；§7 自动化↔手动分工对照 |
...
- **`xml.type` 是协议参数的唯一真相（方案 A，2026-09-21 定稿）**：`soap` 段**不含 `version`**（版本由 type 携带，避免双真相）；**`type=POX` 带 `soap` 键 → `40001` 互斥**；`type` 缺省 = POX（B1 旧数据零迁移）。前端**只输出非空键**、非 XML 出站**恒不提交**（`protocolParamsForPayload(protocolOut, …)`，**只看 `protocol_out`** —— 入站不解包，`in=XML/out=JSON` 时给入口就是「配了不生效」）。
...
- **里程碑计划**（`src/main/resources/doc/开发文档/`）：`M3开发计划.md` / `M4开发计划.md`（均已实施）/ `M5开发计划.md`（已评审定稿，下一步）/ **`B2完整SOAP开发计划.md`（延后待真实样本）** / 各里程碑手动验收方案 + **`B1协议参数手动验收测试方案.md`** + `M5压测报告.md`（M5.3 压测方案与报告，待执行回填）+ 代码评审记录

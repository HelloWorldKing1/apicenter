...
| **入站鉴权 B5（2026-09-23）** | **《入站鉴权设计方案》v1.1 第五批「文档与验收」已落地**：**§17 的 8 处文档同步**（设计方案总纲 §1.2/§3.1/§5.3 口径改写 + §6.2 错误码 **+40107/40108**；时序图出站时序与图注（"组件范围外 Noop 占位"→"网关闸门已实现"）；项目说明 §2 边界 →「**做鉴权、不做授权**」；使用教程 **§4.4「调用方管理」**（四种方式对照 + 凭证轮换 + fail-closed 提示 + 应急回退两种粒度）+ §9.1 带鉴权头 curl 与**三步取证**；CLAUDE.md 表计数/页码/区块数 + 4 条 Gotchas；整体测试方案 **§6.7 A 组 13 例** + **R7**，R1–R6 引用同步为 R1–R7）+ **seed 演示调用方**：`DEMO-CLIENT` + `ADP-401`（调用方 API Key 验签）+ 固定演示密钥（幂等、与 fastmoss 种子**解耦**，在 `importSeed()` 早期补齐 ⇒ 增量导入场景也生效）+ **新增《开发文档/验收/入站鉴权手动验收测试方案.md》（v1.0，页面操作版，约 25 分钟）**：四阶段（观察 / 配置与灰度 / 强制与联动 / 回调与平台自调）+ **灰度操作手册（S0–S4 + 两条硬要求：禁跳步、进 ENFORCED 前需逐个调用方"不带凭证存量 = 0"）** + 10 条常见误判 + 分工表。**全量测试已验（2026-09-23，用户手动执行）：405 例全绿**（首跑 404 例 1 红 → 定位为闸门偏离真值表的真 bug，修复见下条 Gotcha + `5561fc2`，复跑全绿）；**未做**：B6（可选）回调验签契约迁移 |
| **入站鉴权模型修订 v1.2（2026-09-24；**C1–C5 全部落地**）** | 《入站鉴权设计方案》升 **v1.2**：v1.1 的「**先登记调用方，才准调**」改为「**持有平台签发的凭证就准调，登记是可选的精细管控**」。**决策依据（用户三点确认）**：① 调用方是**未知的、会持续新增的第三方**（开放集）；② **需要**「只吊销某一个调用方」；③ 「调用方管理」页**保留并降级**。**四项修订**：① **D-CA-18 入站鉴权凭证池** —— `client_credential` 升级为**三级属主**（`INTERFACE`→`PLATFORM`→`CLIENT`，**逐级短路**）+ **`label`/指纹**（可归因、可单独吊销的抓手）；② **D-CA-19 方式归属** —— 鉴权方式由 `client_app.auth_adapter_id` 改为**接口绑定角色 `CLIENT_AUTH` → 平台默认**（复用 `interface_adapter_binding` ⇒ 快照/版本 diff **零额外改动**）+ `X-Client-Id` 降为**可选自报**（`require-client-id=false`）；③ **§6.2 真值表重画**（判定中心 = **凭证是否命中池**；「停用调用方」= 让其凭证不参与命中）；④ **D-CA-20 审计 +2 列**（`credential_label`/`credential_fingerprint`；**不得做成指标标签**——基数爆炸）。**已知代价（非缺陷）**：`GatewayGuard` 限额仍按 `app` ⇒ **额度为所有调用方共享**（一个滥用者能吃掉整接口额度）⇒ 需要按调用方限流时再升级 L2 档案。**实施计划 C1–C5（§19.2，5.4 人日）**：C1 数据与凭证池（迁移 +3 列/−1 列 + 池端点）/ C2 鉴权内核重写（**兼容档 `require-client-id=true` 旧用例必须全绿**）/ C3 方式绑定 `CLIENT_AUTH` / C4 审计 +2 列（**三处同步**）/ C5 管理面与**验收方案升 v3.0**。**B1–B5 代码是增量改造的基底，不推倒**。**追加确认（用户 2026-09-24）**：平台默认鉴权方式与「是否强制自报主体」**不允许只存配置、必须页面可改** ⇒ 二者**落库**（**新增第 26 张表 `inbound_auth_setting`（单行）**，改后**即时生效**：缓存 + 专用事件 `InboundAuthSettingChangedEvent` + 60s TTL 兜底；放松类变更需二次确认 + 告警 + `updated_by` 留痕）；`mode`（应急总开关）**仍留配置**（改它需重启 = 有意动作）⇒ **表数将由 25 → 26**（落地时需同步 schema/HTML/表数断言）。**C1 已落地（2026-09-24）**：开发库已迁移（+`owner_type`/`owner_id`/`label`、换索引、回填；`client_id` 先放宽为 NULL）并新建第 26 张表；代码新增 `CredentialOwner.{INTERFACE,PLATFORM}`（池谓词 `owner_column`+`<=>` 兼容 NULL）、`InboundCredentialService` + `/api/admin/inbound-credentials`（三级属主 CRUD + `label`）、`InboundAuthSettingRepository/Service` + `/api/admin/inbound-auth/settings`（**单行表 + 缓存 + 专用事件 + 60s TTL**，改后即时生效）、`CredentialRow.label`、`AlertService.recordInboundAuthSettingChanged`；测试 **14 例新增全绿**（池隔离 / 单独吊销 / 迁移不变量 / 设置即时生效 / 参考校验），受影响 72 例回归全绿。**抓到一个真缺陷并修**：`ClientAppRepository.deleteCascade` 仍按旧列 `client_id` 删凭证 ⇒ v1.2 池行（该列为 NULL）会**静默残留**（已改按属主谓词 + 加回归用例） 
> **C2 已落地（2026-09-24）**：`ClientAuthVerifier` **以凭证为中心重写**（① 可选自报主体 → ② **方式三级回退**：接口绑定 `CLIENT_AUTH` → 平台设置 → 命中档案 → ③ 档案 IP 名单 → ④ **凭证池逐级短路** `INTERFACE`→`PLATFORM`→`CLIENT` → ⑤ 适配器校验）；4 个适配器改为消费「候选凭证（id + 明文 + 备注 + 指纹）」并在命中时回写 `credential_label`/`credential_fingerprint`（落审计，D-CA-20）；`ClientIpWhitelistVerifyAdapter` 名单改读 **params**（不登记调用方也能用 IP 方式）。**本批 137 例全绿**（新增 6 单测 + 1 端到端开放集 + 1 IP-params；回归 M1/M2/M3/M4/MonitorStats/HttpErrorSemantics/ClientAuth/池/设置）。**两个真陷阱**（已记入设计方案 §20）：① YAML 裸写 `mode: OFF` 被解析成**布尔 false** ⇒ 平台路径**全量 500**（枚举字符串必须加引号）；② `ClientAuthVerifier` 类是 `@EnableConfigurationProperties(ClientAuthProperties.class)` 的**注册点**，重写时丢掉注解 ⇒ 上下文起不来、所有 `@SpringBootTest` 全红。  > **C3 已落地（2026-09-24）**：① 绑定角色新增 **`CLIENT_AUTH`**（`InterfaceService.ROLES`；**仅出站中转接口可用**，入站回调绑它 → `40001`，与 `AUTH`/`CALLBACK_AUTH` 的方向互斥同构）；② 接口详情新增「**入站鉴权方式**」下拉（只列 4 个入站验签实现，未选 = 继承平台默认）；③ 新增 **「入站鉴权」页**（`views/InboundAuth.vue`，路由 `/inbound-auth`，菜单 ◉）：平台设置（默认方式 + 强制自报主体 + **影响面预览** + **放松类二次确认**）与凭证池（三级属主查询 + 发放/激活/完成轮换/吊销/改备注/删除）；④ `utils/adapterUsage.mjs` 扩 `CLIENT_AUTH` 角色（**只认 4 个已知 impl** —— 未知 impl 在闸门必然 40108，故选择器里就不给选；这 4 个仍可绑 `CALLBACK_AUTH`）+ 新增 `utils/inboundAuth.mjs`（差异判定/文案/属主口径，7 单测）。**测试**：`ClientAuthGateIntegrationTest` **8 例**全绿（含**改绑定即时生效的反证**：未绑 40108 → 绑上 200 → 解绑又 40108；入站接口绑 `CLIENT_AUTH` → 40001）；前端 **135 单测 + SSR 23/23 + lint 0 警告 + build 通过**（SSR 替身补渲染 header/footer 插槽，避免「组件接上了但断言看不到」）。 > **C4 已落地（2026-09-24）**：审计查询新增 `credential` 筛选（**备注或指纹任一匹配**：`AccessAuthLogRepository.where` + `MonitorController`），
> Monitor「接入鉴权」Tab 新增「**凭证（命中）**」列 + 主体列 **「自报未验证」标记** + 凭证筛选框（展示口径抽到 `utils/inboundAuth.mjs#credentialAttribution`）。
> ⚠️ **主动修掉一个 v1.2 引入的真风险**：自报主体会让**指标标签基数无界**（伪造随机 `X-Client-Id` ⇒ Micrometer 无限建标签）——
> 现在只有**已登记主体**才用自己的 id 作 `principal` 标签，其余一律 `unverified`；`AlertService` 的失败窗口**键数上限 2000 + overflow 桶**兜底。
> 偏差记录：告警的「指纹分桶」经评审**驳回有据** —— 拒绝请求基本**拿不到指纹**（没命中凭证/时间戳/防重放/IP 名单等路径都无匹配项），原设想是空桶；替代信号（按指纹的**通过量**异常）记 P2。
> **评审补做（S1–S3）**：① 方式解析**留痕**（审计 reason 标「方式来自哪一级」+ 回退时附前序不可用原因 + `log.warn`）；② `OPTIONAL` 的「是否带了凭证」改用**适配器 params 声明的头名**（可配头名不再被误判为「没带」）；③ 热路径**去重**（同一适配器 id 只查一次库、参数只解析一次）+ 用**查询计数单测**钉住成本（**不引缓存**，保住「每请求读绑定 ⇒ 改绑定即时生效」）。
>
...
**开发文档（37 份 / 7 类）不在本表重复维护** —— 完整清单（含每份的 **当前状态**：✅已落地 / 🟡设计稿 / 📌契约冻结 / 🗂️史料）
与「**我该读哪份**（30 秒决策表）」见 → **`src/main/resources/doc/开发文档/README.md`**（唯一入口）。三条最常走的入口：

| 场景 | 路径 |
|---|---|
| 契约依据（已评审冻结） | `开发文档/契约/M0-01~04` |
| 当前计划与里程碑 | `开发文档/计划/`（`M3/M4/M5开发计划.md`、`B2完整SOAP开发计划.md`…） |
| **写代码前先查坑** | `开发文档/记录/技术踩坑记录.md` |
| 手工验收 / Runbook | `开发文档/验收/`（含 `入站鉴权手动验收测试方案.md` v2.0 逐步骤版） |
| 跑测试 / 造数 / 清理 | `开发文档/测试/整体测试方案.md` ← 测试总纲（用例库 + 回归矩阵 R0–R7） |
...
- **入站鉴权的判定中心 = **凭证**，不是「身份」（v1.2，2026-09-24 拍板，写代码前必读）**：调用方是**开放集**（未知、持续新增）⇒ 鉴权只回答「**是否持有平台签发的凭证**」，不回答「你是谁」。落地要点：① **凭证池三级逐级短路**（接口池有可用凭证就不用平台池 —— 实现成 union 会让「接口专属池」失效，反证用例 42）；② **`X-Client-Id` 是自报**（可伪造，**只能用于统计分组，不能用于吊销/配额**；UI 必须标 `UNVERIFIED`）——**唯一不可伪造的抓手是命中的凭证**（`credential_label` + `credential_fingerprint`）；③ 「停用某调用方」= **吊销其凭证**（不再靠 `client_app.status` 拦主体，否则开放集路径上根本没有档案可拦）；④ `require-client-id` **默认 `true`**（保持 v1.1 行为；改 `false` 是**放松**动作，必须显式）；⑤ `credential_label` **禁止做 Micrometer 标签**（运维输入 ⇒ 基数爆炸）；⑥ **指标/告警里的「主体」标签必须有界**：v1.2 主体可自报 ⇒ 只有**已登记主体**（`principalType=CLIENT`）才用自己的 id 作标签，其余一律 `unverified`（否则伪造随机 `X-Client-Id` 会让 Micrometer 无限建标签；`AlertService` 失败窗口另有 2000 键上限 + `overflow` 桶）；⑦ **接口级入站鉴权方式 = 绑定角色 `CLIENT_AUTH`**（`InterfaceService.ROLES`，**仅 OUTBOUND 可用**，入站回调绑它 → `40001`）；闸门**每请求**读该绑定 ⇒ 改绑定即时生效（无需事件/缓存）；⑧ 页面入口：「入站鉴权」页 = 平台设置（可改即生效）+ 凭证池（三级属主），「调用方管理」页已降级为可选的精确管控。
...
- **请求参数快速导入（2026-09-12）**：接口弹窗 →「请求参数」→ 每侧 `⇪ 快速导入参数`（`components/ParamImportDialog.vue` + `utils/paramImport.mjs`）。示例值取 **token 原始切片**（19 位数字等保真），`JSON.parse` 只用于校验；路径 = 嵌套 `.` + 数组 `[0]` + 特殊键名 `["a.b"]`；默认覆盖同名并追加 + 一次撤销快照；上限 200 条 / 深度 6。设计见 `doc/开发文档/设计/参数快速导入设计方案.md`。
...
> **完整文档清单与状态**：**`src/main/resources/doc/开发文档/README.md`**（唯一入口，含决策表 + 37 份的状态列）。
> 下面只列**现行设计**（事实来源，稳定）与三个常去的资源目录。

- **现行设计**（`src/main/resources/doc/`）：`API中心设计方案.md`（总纲）→ `技术架构和实现方案.md` / `可行性报告.md` / `表结构设计.html` + `schema.sql`（实现四件套）→ `API中心时序图与流程图.md` / `API中心原型.html`（流程与交互）→ `开发计划.md`（M0–M5 里程碑）→ `API中心项目说明.md` / `API中心使用教程.md`（**面向使用者**）
- **开发文档**（`src/main/resources/doc/开发文档/`，7 类）→ **先看其 `README.md`**（状态列 + 决策表）；分类为 `契约/` · `设计/` · `计划/` · `验收/` · `测试/` · `演示/` · `记录/`
- **SOAP 真实样本**（`src/test/resources/soap-samples/`）：6 个公网免密钥 SOAP 服务的 WSDL + 成功/Fault 报文 + `header/`（企业级 Header 样本，G1 关闭证据）
- **测试夹具**（`src/test/resources/`）：`b1-protocol-params/` · `protocol-matrix/` · `m5-load/` · `m3-manual-stubs/` · `m4-manual-stubs/`
- `README.md` — 项目索引

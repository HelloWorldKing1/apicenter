...
sequenceDiagram
    ...
    Grp-->>管理员: 分组创建成功
    管理员->>Api: 新建接口（应用（供应商）→ 分组；类型 / 方法 / 路径 / 协议 / 上游路径）
...
> 鉴权适配器为「应用级默认 + 接口级覆盖」；应用即供应商（归属=上游）；接口归属经应用 → 分组选择；协议适配器按入站/出站协议自动推导；字段映射在接口级配置（平台侧字段 → 上游侧字段）。
...
sequenceDiagram
    ...
    Note over GW,链: 链顺序：入站鉴权 → 协议解码 → 报文适配 → 字段映射 → 协议编码 → 出站鉴权
    SP->>GW: POST /callback/{appId}/order-status（X-Partner-Signature / X-Timestamp）
    GW->>链: 入站鉴权适配器（应用密钥验签）
    alt 验签失败
        链-->>SP: 401 签名无效
    end

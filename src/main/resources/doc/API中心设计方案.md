...
### 1.1 应用模型与生命周期
- 应用 = 平台对接的一方供应商（第三方供应商，如腾讯云 / 阿里云 / 智慧芽），持有 appId/appSecret；平台代理调用该供应商的接口。
- 供应商即供应商：接口归属某供应商，也转发到该供应商，二者合一，不再区分「归属应用」与「供应商应用」。
...
- 核心字段：接口标识、接口类型（出站中转 / 入站回调）、HTTP 方法、平台侧路径（如 /api/orders、/callback/{appId}/instance-state）、应用（供应商，appId，既是归属也是供应商）+ 目标地址（出站 = 供应商接口路径 path；入站 = 回调地址 deliveryUrl）、描述。
- 归属：接口属于某应用下的某分组（应用 → 分组 → 接口），新建时经两级下拉选择。
- 鉴权：出站中转接口只配「供应商签名」（出站鉴权，应用级默认 + 接口级覆盖）；入站回调接口只配「回调验签」（入站鉴权，应用级默认 + 接口级覆盖）；调用方鉴权 / 向回调地址签名由平台统一处理（**2026-09-24 v1.2 修订**：鉴权方式**可**在接口上绑定 `CLIENT_AUTH`，见《开发文档/设计/入站鉴权设计方案.md》v1.2 D-CA-19；凭证仍不在接口模型内）（**调用方鉴权已于 2026-09-23 实现**：网关入口闸门 + `client_app`/`client_credential`，见《开发文档/设计/入站鉴权设计方案.md》；向回调地址签名仍未做）。
...
### 3.2 接口归属与适配器链
- 接口可挂**前置步骤（编排，2026-09-18 落地）**：仅出站中转接口，按 `seq` 串行复用其他出站接口（取 token / 主数据 / 额度校验等），结果合入保留命名空间 `steps.<步骤名>.<字段>` 供本接口的字段映射与 condition 引用，再调本接口自己的供应商；复用前置接口的链 / 凭证 / 短重试 / 熔断，长重试（补偿）仍由宿主状态机驱动（前置调用**不落子记录**，避免孤儿重放）。约束：目标必须为出站接口且已发布；步数 ≤5；解析链 ≤3 层（含宿主）；一期失败策略仅 ABORT（阻断后续，未过即宿主链失败，不推进状态机）；被引为前置的接口禁止删除，下线时服务端回 `warnings[]` 强提示。详见《开发文档/设计/前置接口编排设计方案.md》。
- 接口归属唯一应用（经分组），父子关系，不再多对多授权。
- 接口可绑定鉴权与报文适配器，未绑定时继承应用默认；协议适配器按接口入站 / 出站协议自动推导；字段映射为接口级配置（见 3.1 / 5.6）。

### 3.3 接口级配置（超时 / 重试）
- 读超时（默认 **10000ms**，值域 **100~60000ms**，越界 40001；2026-09-22 起默认 3000→10000）：出站 = 调供应商超时；入站 = 回调地址调用超时。
  - 实现（D-PS-0，2026-09-13 修复）：`PerRequestReadTimeoutFactory` 按超时值缓存委托工厂 + `UpstreamInvoker.dispatch` 作用域声明；每次调用（含其内部短重试）按该接口配置生效；≤0 / 未声明作用域 → 兜底 `app.api-center.default-read-timeout-ms`（默认 **10000ms**，不支持「不限超时」）。修复前该配置被全局硬编码 3000ms 静默忽略。
  - 连接超时为全局 `app.api-center.connect-timeout-ms`（默认 3000ms）：per-request 连接超时需 per-request `HttpClient`，成本高收益低。
- 重试策略：最大重试次数（默认 4，值域 **0~10**，越界 40001——上限须小于注解层 `RETRY_CAP=16`，否则配置会被静默截断）、退避、重试条件（5xx/429）。
...
- 熔断：供应商持续失败时快速失败，跳过重试直接进补偿 / 死信（详见 6.4）。
- 5xx/429 短重试 → 耗尽补偿。
- 4xx 死信。
- 超时 → UNKNOWN 对账。
- 补偿 worker 定时扫描。
...
### 4.3 链路追踪
- OpenTelemetry 集成，span 贯穿平台→供应商。
- 可定位到具体节点耗时与失败。
...
- 统一适配器接口，可插拔、可扩展。
- 适配器链：请求按固定顺序流过「入站鉴权（验来源：调用方 / 供应商回调）→ 协议解码 → 报文适配 → 字段映射 → 协议编码 → 出站鉴权（向供应商 / 回调地址附加凭证）」；其中调用方鉴权与向回调地址签名由平台统一处理（**2026-09-24 v1.2 修订**：鉴权方式**可**在接口上绑定 `CLIENT_AUTH`，其余仍不在接口模型内；**调用方鉴权已实现为网关闸门**，见《开发文档/设计/入站鉴权设计方案.md》）。
...
- 回调验签（供应商 → 平台，仅入站回调接口）：
  ...
  - 调用方鉴权（平台自己的客户，如 ERP）由平台统一处理 —— **2026-09-23 已实现**（**2026-09-24 v1.2 修订**：判定中心 = 「是否持有平台签发的凭证」（**不登记也能调**）、方式**三级回退**（接口绑定 `CLIENT_AUTH` → 平台默认 → 命中档案）、凭证进**三级属主凭证池**（接口 → 平台 → 档案，逐级短路）、平台默认与「强制自报主体」页面可改即生效 ⇒ 「不在本模型内」仅对**凭证**成立）：网关入口闸门（`GatewayController` 内、引擎前）+ 新主体 `client_app` / `client_credential` / `access_auth_log`，四种方式（API Key / HMAC / Bearer / 仅 IP 名单）、三态灰度 `client-auth.mode`，详见《开发文档/设计/入站鉴权设计方案.md》v1.1。
...
- 报文结构转换：信封/包裹、报文头处理。
- 响应信封映射：剥供应商信封（`envelope`，如 `data`）+ 读供应商状态码（`codeField`/`successValue`）判断成败 + 错误码映射（`codeMappings`）+ 包平台统一信封 `{code, msg, data}`（`msg` 透传供应商 `messageField`）。
...
| 报文 · 信封（EnvelopeMessageAdapter） | `envelope`、`codeField`、`successValue`、`codeMappings[]`、`messageField`、`defaultErrorCode` | 业务数据容器（data）、供应商状态码字段、成功值、错误码映射、消息字段、兜底错误码 |
...
错误码映射 `codeMappings[]` 每条：`from`（供应商错误码）、`to`（平台错误码）。
...
stateDiagram-v2
    [*] --> INIT
    INIT --> MAPPING: 字段映射
    MAPPING --> SENDING: 调供应商
    SENDING --> SUCCESS: 供应商成功
...
|---|---|---|
| 0 | 成功 | — |
| 401xx | 鉴权失败 | 40100 验签失败（签名不匹配 / 缺头 / 防重放重复）、40101 时间戳超容差、40102 应用未启用、40103 来源 IP 被拒（黑名单命中 / 白名单未命中）、**40107 调用方不存在或已停用**、**40108 调用方未配置鉴权方式 / 无可用凭证**（后两条 2026-09-23 入站鉴权落地，见《开发文档/设计/入站鉴权设计方案.md》） |
...
| 429xx | 限流 / 配额 | 42901 QPS 限流、42902 日配额超限、42903 供应商限流（透传供应商 429） |
| 502xx | 供应商错误 | 50201 供应商 5xx / 429 重试耗尽转补偿、50202 熔断短路、**50203 SOAP Fault（客户端类：Client/Sender/VersionMismatch/MustUnderstand → 死信、不重试、不计熔断）** |
| 504xx | 供应商超时 | 50401 供应商读超时 |
| 500xx | 平台内部错误 | 50000 未知异常 |

> 说明：供应商返回 429 属限流语义，归入 429xx（42903 透传），重试耗尽则落 50201 转补偿；50202 为平台侧熔断短路（非供应商错误）；504xx 与 502xx 的 HTTP 映射遵循「业务码 / 100 = HTTP 状态」约定。

### 6.3 对账 / 补偿机制

- 对账（UNKNOWN 处理）：UNKNOWN = 结果不确定（可能已达供应商），不可盲目重试；已成功 → 收敛 SUCCESS，未到达 → 触发补偿 COMPENSATING。M4 现状：人工置位（source=MANUAL）与 TTL 超时自动降级（默认 10 分钟，source=TTL）双来源，均落 reconcile_audit 审计；按供应商查询接口自动对账（v1.1）排期未做。
- 补偿：补偿 worker 定时扫描出站 COMPENSATING 记录与入站 PENDING 记录，按固定间隔（如 3s）重试；超过最大次数（如 5 次）转死信 + 告警；补偿重放不重复生效依赖**供应商对业务键幂等**（请求携带稳定 biz_id 由供应商去重）。

### 6.4 熔断机制

- 目标：供应商持续不可用时快速失败，避免反复重试压垮供应商、占用线程与连接资源。

...
CREATE TABLE app (
    ...
    base_url                    VARCHAR(255) COMMENT '服务地址（供应商 API 根地址，出站接口的供应商接口路径拼于此）',
...
CREATE TABLE outbound_request (
    ...
    biz_id        VARCHAR(64) NOT NULL COMMENT '业务键（请求携带稳定 biz_id，供应商据此去重）',
    ...
    error_code    VARCHAR(32) COMMENT '错误码（50201 供应商 5xx / 50401 超时等）',
...
CREATE TABLE outbound_request_state_log (
    ...
    detail              VARCHAR(500) COMMENT '补充说明（死信编号 / 操作人 / 短重试次数 / 供应商 5xx 等）',

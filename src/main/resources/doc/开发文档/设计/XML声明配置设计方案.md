...
> 📋 **执行计划已单独成文**：**《B2完整SOAP开发计划.md》**（含开工 gate / T1~T11 任务拆解 / 用例矩阵 / 手动验收三阶段 / 出口检查表）。
> 本节只保留人日汇总（与那份计划一致：**6.4 人日**）。
...
| **Q19** | SOAP 是否有真实供应商支撑 | **B2 延后**，先上 B1；触发条件 = 拿到真实 SOAP 供应商 WSDL/报文样例（建议 30 天内否则正式延后） | §9 实施顺序、§11 批次归属、§13.1 #11、**《B2完整SOAP开发计划.md》** |
...
> 🧪 **手动复验入口**：**《协议参数手动验收测试方案.md》**（**五阶段约 30 分钟**，页面操作版：B1 四阶段 + **B2 SOAP 阶段五**，含基线预期值与“常见误判”表）。

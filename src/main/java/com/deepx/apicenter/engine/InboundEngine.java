...
public class InboundEngine {
    ...
    public void redeliver(InboundDeliveryRow row) {
        inboundDeliveryRepository.incrementAttempt(row.id());
    ...
    private void deliver(long deliveryId, InterfaceRow iface, OutboundRequestSpec spec) {
        ...
        if (!callbackUrlValidator.isAllowed(spec.url())) {
            inboundDeliveryRepository.updateState(deliveryId, "DEAD_LETTER", null);
            insertDeadLetterOnce(deliveryId, "回调地址安全校验失败（SSRF）", payloadText(spec.body()));
        ...
        try {
            ...
            inboundDeliveryRepository.updateState(deliveryId, "PENDING",
                    LocalDateTime.now().plusSeconds(RETRY_INTERVAL_SECONDS));

...
```bash
mvn test            # 全库 276 个 @Test（集成测试连开发库；成本高时用 -Dtest=<类> 跑针对性批次）
...
npm test            # 单测 73 例 + 组件 SSR 冒烟 18 例（Node 内置 test runner，无需联网）
...
**两条安全底线（服务端强制）**：① 不能停用/删除**最后一个可用账号**（否则谁也登不进来）；
② 不能停用/删除**当前登录的自己**、不能用「重置密码」改自己的口令。违反返回 `40001` 并给出原因；界面对应按钮置灰。

...
public record AdminUserRow(
    ...
    /** 对外视图（无口令摘要；含角色，前端据此控制菜单与按钮） */
    public record View(long id, String username, String displayName, String role, LocalDateTime lastLoginAt) {
        public static View of(AdminUserRow row) {
            return new View(row.id(), row.username(), row.displayName(), row.role(), row.lastLoginAt());
        }
    }
     ...
     */
    public record ListRow(long id, String username, String displayName, String role, String status, int failedAttempts,

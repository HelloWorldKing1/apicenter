...
- **做调用方鉴权、不做授权**（2026-09-23 落地）：调用方（平台客户）在网关入口按 `client_app` 主体鉴权（API Key / HMAC-SHA256 / Bearer / 仅 IP 名单四种方式），三态灰度 `client-auth.mode`（`OFF` 默认 → `OPTIONAL` → `ENFORCED`），每次判定写 `access_auth_log` 审计（主体名 · IP · 方式 · 结果）；**授权**（能调哪些接口）与临时封禁不在本版本；调用方鉴权**不在接口模型内**（不进接口配置/快照）。
...
| `repository/` | JdbcTemplate 数据访问（25 张表，无 JPA） |
...
共 **25 张表**（配置 12 + 运行 8 + 管理面账号 2 + **入站鉴权 3**；其中 interface_step 为前置步骤编排、admin_user/admin_session 为管理面账号登录（2026-09-18）、client_app/client_credential/access_auth_log 为平台入站鉴权（2026-09-23），无数据库外键（引用完整性应用层保证，引用列建索引）。DDL 见 [`schema.sql`](schema.sql)，可视化见 [`表结构设计.html`](表结构设计.html)。

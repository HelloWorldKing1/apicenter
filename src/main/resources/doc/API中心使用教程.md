...
数据库已按 `src/main/resources/doc/schema.sql` 建好（**26 张表**）。首次在全新库上部署时：
...
### 4.4 入站鉴权（平台怎么验调用方，v1.2 · 2026-09-24）

> **一句话**：**给调用方发一把密钥就行**（不需要为对方建档）。侧边栏「**◉ 入站鉴权**」页做两件事：
> **① 平台设置**（平台默认鉴权方式 + 是否强制自报主体）；**② 凭证池**（三级属主：平台共享 / 接口专属 / 调用方档案）。
> 另有「**调用方管理**」（本节末尾）作为**可选的精确管控**。

> **谁能用这个页面**：**ADMIN / OWNER**（只读角色 VIEWER 看不到菜单、接口也会 403）；其中**修改「平台设置」
> 仅 OWNER 可做**（改默认方式/强制自报主体 = 影响全平台安全策略），ADMIN 可正常发放与吊销凭证。

**从零接入一个调用方（推荐路径，1 分钟）**：
1. 「适配器」页确认有一个入站验签适配器（如「调用方 API Key 验签」；种子里有 `ADP-401`）；
2. 「入站鉴权」页 → **平台设置** → 选「平台默认鉴权方式」→ 保存（**改后立即生效，无需重启**）；
3. **凭证池** → 属主选「**平台共享池**」→「＋ 发放新凭证」→ 类型 `API_KEY`、**备注**填「发给谁 / 何时」→ 生成；
4. 点该行「**激活**」（新凭证先是 `ROTATING` 待激活 —— **最容易忘的一步**）；
5. 把**明文**（只显示这一次）发给调用方。对方调用时带：

```bash
curl -X POST http://localhost:8080/<平台侧路径> -H 'Content-Type: application/json' \
  -H 'X-Api-Key: <明文>' -d '<请求体>'
```

| 方式（适配器） | 调用方要带什么 | 需要的凭证类型 |
|---|---|---|
| **调用方 API Key 验签** | `X-Api-Key: <密钥>` | `API_KEY` |
| **调用方 HMAC 验签** | `X-Timestamp` + `X-Signature`（签名口径**与供应商回调验签完全一致**） | `HMAC_SECRET` |
| **调用方 Bearer 验签** | `Authorization: Bearer <token>`（`prefix` 可置空 = 裸 token） | `BEARER_TOKEN` |
| **调用方 IP 名单** | 只需来源 IP（名单填在**适配器参数**里） | ❌ 不需要 |

**几个必知口径**：
- **鉴权方式的三级回退**：**接口绑定**（接口管理 → 详情 →「入站鉴权方式」下拉，`CLIENT_AUTH`）→ **平台默认**（本页设置）→ 命中档案的方式（兼容旧配置）。都没有 ⇒ `40108`（**fail-closed，绝不回退"不校验"**）。
- **凭证池逐级短路**：`接口专属池` → `平台共享池` → `档案池`。**某一级有可用凭证就只用这一级** ——
  所以给接口建了专属池后，平台共享密钥在**该接口**上会被拒（这是"只给某几个调用方开放"的实现方式）。
- **单独吊销**：一凭证一行，**吊销其中一把不影响其他**（无需改配置、无需重启）—— 这就是「共享密钥」与「凭证池」的区别。
- **`X-Client-Id` 是自报的**（可选）：v1.2 起它**不参与放行判定**，只用于审计归属；审计里会标「**自报未验证**」，
  **不能**当作身份用于追责/配额。真正不可伪造的是**命中的凭证**（审计「凭证（命中）」列 = 备注 + 指纹）。
- **两种拒绝要分清**：`40100` = 密钥不对；`40108` = 平台/接口侧**没有可用凭证或没配方式**（运维问题，不是调用方的错）。
- **审计查询**：「接口监控 → 接入鉴权」可按主体 / IP / 结果 / 方式 / **凭证（备注或指纹）**筛。

> ⚠️ **`mode` 仍是"启动期"开关**（`app.api-center.client-auth.mode`，改后**必须重启**）：它是上线与应急总开关，
> 刻意保留"有意动作"语义；而上面的**平台默认方式**与**强制自报主体**是页面可改、即时生效的。

#### 4.4.1 可选：调用方管理（精确管控，2026-09-23）

> 什么时候才需要它？① 该调用方要**可验证身份**（合规/追责）；② 要**单独 IP 名单**；③ 将来要**单独配额**（P2）。
> 其余情况用上面的**凭证池**更直接。
...
**字段路径与层级嵌套（`source` / `target` 都支持点路径，2026-09-24 补充口径）**

| 能力 | 写法 | 结果 |
|---|---|---|
| **扁平 → 多层嵌套** | `source = a.id`，`target = b.data.id` | `{"a":{"id":1}}` ⇒ `{"b":{"data":{"id":1}}}` —— **中间层自动创建**，不必先造空对象 |
| **嵌套 → 扁平** | `source = a.b.c`，`target = flat` | `{"a":{"b":{"c":"x"}}}` ⇒ `{"flat":"x"}` |
| **深层 → 更深** | `source = x.y`，`target = x.y.z` | 同名对象**原地追加**（不会把 `x` 冲掉） |
| **XML 嵌套 ⇄ 模型嵌套** | 无需额外配置 | 入站 `<data><id>7</id></data>` ⇒ 模型 `{"data":{"id":"7"}}`；出站模型 `{"data":{"id":7}}` ⇒ `<request><data><id>7</id></data></request>`（**根元素**由「XML 协议参数」的根元素决定） |
| **同名重复元素 → 数组** | 无需额外配置 | `<items><item>…</item><item>…</item></items>` ⇒ `items.item` 为**数组**（点路径取 `data.items.item`） |
| ⚠️ **数组下标寻址** | `list[0].id` **不支持**（M0-02 D1 决策） | 该写法会被当成字面键去查 ⇒ 取不到值（按 `nullStrategy` 处理，**不会报错也不会猜下标**）。元素级转换请用 **`aggregate`**（`SUM`/`MAX`/`MIN`/`CONCAT[:分隔符]`）；单个元素的字段要另取，可在**协议适配器**侧调整或用上游返回的平铺字段 |
| ⚠️ **键名本身含点（`.`）** | 无特殊语法 | 点号一律按**层级分隔符**解释 ⇒ 键名里带点的字段**无法用路径寻址**（建议在源/目标侧避免这种键名） |
...
**若平台启用了「入站鉴权」**（`app.api-center.client-auth.mode` 非 `OFF`），要带鉴权头 ——
**凭证明文来自「入站鉴权」页的凭证池（或「调用方管理 → 凭证」）**，生成时**只显示一次**：

```bash
# ① API Key 方式
curl -X POST http://localhost:8080/<平台侧路径> \
  -H 'Content-Type: application/json' \
  -H 'X-Client-Id: <可选：调用方标识；v1.2 起只为审计，不参与放行>' \
...
**关于「记住用户名」与「记住密码」**（2026-09-24）：
- 「**记住用户名**」**默认勾选** ⇒ 下次自动填好用户名；取消勾选会**同时清掉**已记住的值。
- 「**记住密码（本机加密）**」**默认不勾选**，勾选时会弹**风险确认**（请读完再决定）。它的口径：
  · 口令**不是明文落盘**：用浏览器 WebCrypto 的 **AES-GCM** 加密，密钥**不可导出**且存本机 IndexedDB，
    密文另存 —— 把密文拷到别的浏览器/设备**解不开**；
  · 但它**防不住同源脚本（XSS）**，也防不住已能操作这台电脑的人（可开 DevTools / 注入脚本）
    ⇒ **仅建议个人专用设备开启**，公用/共享电脑请勿勾选；
  · 取消勾选会**真删**（密钥 + 密文）；**修改密码成功后会自动清除**已记住的旧口令；
  · 浏览器不支持时（无 WebCrypto / 无 IndexedDB）该复选框会**禁用**并说明原因 —— 这是有意的
    （宁可不记住，也不退回明文保存）。
- **更推荐**：让**浏览器自带的密码管理器**接管（首次登录成功后浏览器会提示「保存密码」，下次自动填充，
  还可跨设备同步）。若没弹出：检查浏览器是否禁用了密码保存、或处于隐私/访客模式。
- **免重复登录**：令牌有效期 12 小时且**每次请求会惰性续期** —— 只要你在用就不会被中途踢出；
  因此没有「记住我 30 天」这类长会话开关（长会话只增加令牌被盗后的利用窗口）。

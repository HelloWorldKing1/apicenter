...
- **响应方向（M3 已收敛回协议适配器，原 M2 技术债）**：上游响应字节经 `ChainEngine.decodeResponse` 按 `protocol_out` 推导的协议适配器解码（与请求方向同一条 DECODE 路径，JSON/XML 一个分支），再交信封适配器剥壳/判成败，随后经 RESP 白名单过滤（D-M3-3）回统一信封。错误响应（4xx/5xx/超时）按 HTTP 状态分类、不解码为业务模型。

...
> 图二注（**2026-09-23 修订**）：调用方鉴权**已在本组件实现**（网关入口闸门：`X-Client-Id` + API Key / HMAC / Bearer / 仅 IP 名单），由 `client-auth.mode` 三态控制（`OFF` 默认不校验但仍写审计）；调用方**授权**（能调哪些接口）仍不在本版本。详见《开发文档/入站鉴权设计方案.md》v1.1。

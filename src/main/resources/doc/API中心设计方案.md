...
### 3.3 接口级配置（超时 / 重试）
- 读超时（默认 3000ms，值域 **100~60000ms**，越界 40001）：出站 = 调供应商超时；入站 = 回调地址调用超时。
  - 实现（D-PS-0，2026-09-13 修复）：`PerRequestReadTimeoutFactory` 按超时值缓存委托工厂 + `UpstreamInvoker.dispatch` 作用域声明；每次调用（含其内部短重试）按该接口配置生效；≤0 / 未声明作用域 → 兑底 `app.api-center.default-read-timeout-ms`（默认 3000ms，不支持「不限超时」）。修复前该配置被全局硬编码 3000ms 静默忽略。

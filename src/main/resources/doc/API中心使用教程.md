...
- 登录页有「**记住用户名**」（默认勾选）⇒ 下次自动填好用户名；取消勾选会**同时清掉**已记住的值。
- **口令一律不由本平台保存在浏览器里**：请让**浏览器自带的密码管理器**接管 —— 首次登录成功后浏览器会弹出
  「保存密码」，点保存即可；下次会**自动填充用户名与密码**，还可跟随浏览器账号跨设备同步。
  （若没弹出：检查浏览器是否禁用了密码保存，或地址栏是否处于隐私/访客模式。）
- **免重复登录**：令牌有效期 12 小时且**每次请求会惰性续期** —— 只要你在用，就不会被中途踢出；

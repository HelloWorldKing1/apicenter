...
- **[API中心项目说明](src/main/resources/doc/API中心项目说明.md)** —— 定位 / 核心概念 / 架构 / 两条链路 / 数据模型 / 状态机容错 / 错误码 / 开发状态。
- **[API中心使用教程](src/main/resources/doc/API中心使用教程.md)** —— 环境准备 → 启动 → 从零配置一条接口 → 调用验证 → 监控与容错运维，含界面操作与等价 curl。
...
- **管理面账号（2026-09-18）**：登录 / 注册 / 退出 / 改密（不透明 Bearer 令牌，库内只存 SHA-256 摘要，PBKDF2 口令摘要）+ **账号管理页**（列表 / 新建 / 启停用 / 重置口令 / 解锁 / 删除；两条安全底线：不能动最后一个可用账号、不能动自己）。只做认证，**不做权限**。
- **测试**：全库归属 **290 个 @Test**（2026-09-18 基准；含 D-PS-0 读超时 8+3 例、接口级数值值域 2 例、补偿预算语义 1 例、前置编排 `PreStepIntegrationTest` 15 例、快照 steps 往返 1 例、HTTP 错误语义 4 例、**账号登录 `AuthIntegrationTest` 14 + `PasswordHasherTest` 5**）。受成本约束本轮**只跑针对性批次**（编排 15 / 错误语义 4 / M1 19 / 告警 10 / M2-M5+StateChain+MonitorStats+单测 60，全绿），**未跑全量**；前端 `npm run lint` + `npm test`（单测 83 + SSR 冒烟 18）+ `npm run build` 全绿。
...
- **前置接口编排（2026-09-18）**：出站接口可挂「前置步骤」，按 `seq` 串行复用另一个出站接口（如取 token / 主数据 / 额度校验），结果合入 `steps.<步骤名>.<字段>` 供字段映射引用，再调自己的第三方；复用前置接口的链 / 凭证 / 短重试 / 熔断，长重试由宿主状态机驱动。入口：接口弹窗「前置步骤」Tab（详见《开发文档/设计/前置接口编排设计方案.md》）。
...
| 交付物 | 路径 | 说明 |
|---|---|---|
| 项目说明 | [API中心项目说明.md](src/main/resources/doc/API中心项目说明.md) | 面向使用者的项目总览：定位 / 核心概念 / 架构 / 两条链路 / 数据模型 / 状态机容错 / 错误码 |
| 使用教程 | [API中心使用教程.md](src/main/resources/doc/API中心使用教程.md) | 从零上手：环境准备 / 启动 / 接口配置 / 调用验证 / 监控运维 / FAQ（界面 + curl） |
...
| 端到端演示方案 | [端到端闭环演示方案.md](src/main/resources/doc/开发文档/演示/端到端闭环演示方案.md) | 从界面配置到真实供应商（evoLink）/ 真实 XML 闭环的完整演示脚本；每环节给原理 / 设计思路 / 实现方式 |
| 前置编排真实接口案例 | [前置接口编排真实接口案例（FastMoss + evoLink）.md](src/main/resources/doc/开发文档/演示/前置接口编排真实案例.md) | 用真实供应商（FastMoss 达人列表 / evoLink 生图）做前置接口的手动测试：命名空间引用、数组限制正解、失败码、可判别断言与 stub 回放 |
| 前置接口编排（方案稿） | [前置接口编排设计方案.md](src/main/resources/doc/开发文档/设计/前置接口编排设计方案.md) | **PS-1..PS-9 已落地（2026-09-18）**：`interface_step` 配置子表 + `PreStepExecutor` + `ResponseJudger` + 前端「前置步骤」Tab；含落地记录（§0）、与文档偏差、待拍板项 |
| 代码评审记录 | [doc/开发文档/](src/main/resources/doc/开发文档/) | M2 / M3 四路评审问题清单与修复进度 |
| 踩坑记录 | [技术踩坑记录.md](src/main/resources/doc/开发文档/记录/技术踩坑记录.md) | Spring 7 / Jackson 3 / WireMock 3 API 差异与经验（写代码前先查） |

...
- **账号管理（v1，2026-09-18）只有两条安全底线、没有权限分级**：① 不能停用/删除**最后一个可用账号**（`countEnabled()<=1`）② **不能动自己**（停用/删除/重置口令；改显示名允许）。判定顺序**先①后②**（单账号环境下提示更贴切）。
  v1 无 RBAC ⇒ **任何已登录账号都能管理账号**（页面顶部常驻提示）；引入角色后应收敛到管理员（设计文档 §12/§13）。新增账号管理类端点时：守卫写进 `AdminUserService`（服务端权威），前端 `utils/users.mjs` 只做按钮禁用镜像。
...
- **里程碑计划**（`src/main/resources/doc/开发文档/`）：`M3开发计划.md` / `M4开发计划.md`（均已实施）/ `M5开发计划.md`（已评审定稿，下一步）/ 各里程碑手动验收方案 + `M5压测报告.md`（M5.3 压测方案与报告，待执行回填）+ 代码评审记录

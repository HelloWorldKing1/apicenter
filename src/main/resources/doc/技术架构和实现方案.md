...
- **响应方向（当前为引擎内联解析，M2 技术债）**：上游响应字节在 `OutboundEngine` 内联执行与 `toUnified` 相同的转换（`parseResponse`），再交信封适配器剥壳/判成败，取业务内容回统一信封。该内联逻辑与协议适配器重复、且无 XML 分支——**M3 落地 XML 时应收敛回协议适配器**，使响应解析与请求方向同走一条协议推导路径。
- 适配器目录已含 XML 元数据占位；`XmlProtocolAdapter` 实现与协议推导的 XML 分支为 M3 交付。

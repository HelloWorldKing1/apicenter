...
- **测试**：全库归属 **219 个 @Test**（2026-09-18 基准；含 D-PS-0 接口级读超时 8+3 例、接口级数值值域 2 例、补偿预算语义固化 1 例、前置接口编排 `PreStepIntegrationTest` 12 例、快照 steps 往返 1 例），`mvn test` 全绿；前端 `npm run lint` + `npm test`（单测 48 + SSR 冒烟 16）全绿。

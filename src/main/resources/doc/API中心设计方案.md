...
- 鉴权：出站中转接口只配「供应商签名」（出站鉴权，应用级默认 + 接口级覆盖）；入站回调接口只配「回调验签」（入站鉴权，应用级默认 + 接口级覆盖）；调用方鉴权 / 向回调地址签名由平台统一处理、**不在接口模型内**（**调用方鉴权已于 2026-09-23 实现**：网关入口闸门 + `client_app`/`client_credential`，见《开发文档/入站鉴权设计方案.md》；向回调地址签名仍未做）。
...
- 统一适配器接口，可插拔、可扩展。
- 适配器链：请求按固定顺序流过「入站鉴权（验来源：调用方 / 供应商回调）→ 协议解码 → 报文适配 → 字段映射 → 协议编码 → 出站鉴权（向供应商 / 回调地址附加凭证）」；其中调用方鉴权与向回调地址签名由平台统一处理、不在接口模型内（**调用方鉴权已实现为网关闸门**，见《开发文档/入站鉴权设计方案.md》）。
...
- 回调验签（供应商 → 平台，仅入站回调接口）：
  ...
  - 调用方鉴权（平台自己的客户，如 ERP）由平台统一处理、**不在本模型内** —— **2026-09-23 已实现**：网关入口闸门（`GatewayController` 内、引擎前）+ 新主体 `client_app` / `client_credential` / `access_auth_log`，四种方式（API Key / HMAC / Bearer / 仅 IP 名单）、三态灰度 `client-auth.mode`，详见《开发文档/入站鉴权设计方案.md》v1.1。
...
|---|---|---|
| 0 | 成功 | — |
| 401xx | 鉴权失败 | 40100 验签失败（签名不匹配 / 缺头 / 防重放重复）、40101 时间戳超容差、40102 应用未启用、40103 来源 IP 被拒（黑名单命中 / 白名单未命中）、**40107 调用方不存在或已停用**、**40108 调用方未配置鉴权方式 / 无可用凭证**（后两条 2026-09-23 入站鉴权落地，见《开发文档/入站鉴权设计方案.md》） |

...
基础 API 中转 + 字段映射的统一调用平台：对外暴露统一接口，对内经适配器链完成鉴权、协议转换、报文适配、字段映射，转发至第三方供应商，或将供应商回调送达调用方。**只做连接 + 适配 + 可靠传输，不承接业务决策。**
...
| 链路追踪 | OpenTelemetry（自动埋点 + 适配器节点手动 span） | §4.3 span 贯穿平台 → 供应商，traceId 请求头透传 |
...
#### 4.1.1 JSON 编解码过程（以 M2 出站链路为例）

- **请求方向（走协议适配器 Bean，JsonProtocolAdapter）**：
  1. 调用方原始报文以字节进入链（`ctx.attrs("rawBody")`）；
  2. DECODE 阶段：`readTree(byte[])` → Jackson `JsonNode` → `toUnified()` 递归转**统一内部模型**（对象→OBJECT 保序、数组→ARRAY、整数→INT、小数→DECIMAL、布尔→BOOLEAN、字符串→STRING、null→NULL）；
  3. 报文适配 / 字段映射等阶段只操作这棵树（空映射=原样透传，格式无关）；
  4. ENCODE 阶段：`fromUnified()` 反向递归 → `JsonNode` → `writeValueAsBytes` 写出站报文 + Content-Type；
  5. 解码失败（非法 JSON）→ `40002 报文格式非法`，链失败不落状态机（M0-01 D7）。
- **响应方向（M3 已收敛回协议适配器，原 M2 技术债）**：供应商响应字节经 `ChainEngine.decodeResponse` 按 `protocol_out` 推导的协议适配器解码（与请求方向同一条 DECODE 路径，JSON/XML 一个分支），再交信封适配器剥壳/判成败，随后经 RESP 白名单过滤（D-M3-3）回统一信封。错误响应（4xx/5xx/超时）按 HTTP 状态分类、不解码为业务模型。
- XML 协议适配器 `XmlProtocolAdapter`（Woodstox StAX，D-M3-1 语义）与协议推导 XML 分支为 M3 交付（见 `doc/开发文档/计划/M3开发计划.md`）。
...
- 补偿 worker 定时扫描 COMPENSATING 按间隔重试，超最大次数 → 死信 + 告警；**重放安全依赖供应商对业务键幂等**（请求携带稳定 biz_id 由供应商去重）。
...
- **对账**：UNKNOWN 记录通过查询接口查供应商真实状态（已成功→SUCCESS，未到达→触发补偿）。
...
- **链路**：OpenTelemetry 自动埋点 + 适配器链每个节点手动 span（入站鉴权 / 协议解码 / 报文适配 / 字段映射 / 协议编码 / 出站鉴权）；traceId 以请求头透传「平台 → 供应商 / 回调地址」，与 `outbound_request` / `inbound_delivery` 的 trace_id 关联。
...
- 表：`app` + `app_credential`（+ 分组见 5.2）。
- 要点：
  - 凭证：落 `app_credential` 表（kind=OUTBOUND/CALLBACK，ACTIVE/ROTATING/RETIRED 状态机），AES-256-GCM 加密存储、遮显展示、轮换并存 / 重置 / 即时失效（见 M0-04 凭证轮换存储方案）。
...
- 要点：
  ...
  - 类型约束：出站 = 供应商接口路径 + 供应商签名 + 出站响应字段；入站 = 回调地址 + 回调验签 + 出站侧送达报文（必填）+ ack 回执字段；两侧互斥字段按类型清空。
...
| M2 出站链路 | 适配器注册表 + 链引擎 + 协议编解码（JSON）+ 字段映射全操作 + 出站状态机/短重试/补偿/死信 | json-json 出站链路端到端跑通（出口基准 = fastmoss 黄金用例，见《开发计划.md》§2） |
...
5. **不设平台侧幂等开关**——去重依赖供应商对业务键幂等（补偿重放安全的必要条件）。

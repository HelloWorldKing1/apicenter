...
- **测试**：全库归属 **227 个 @Test**（2026-09-18 基准；含 D-PS-0 读超时 8+3 例、接口级数值值域 2 例、补偿预算语义 1 例、前置编排 `PreStepIntegrationTest` 14 例、快照 steps 往返 1 例、HTTP 错误语义 4 例）。受成本约束本轮**只跑针对性批次**（编排 14 / 错误语义 4 / M1 19 / 告警 10 / M2-M5+StateChain+MonitorStats+单测 60，全绿），**未跑全量**；前端 `npm run lint` + `npm test`（单测 58 + SSR 冒烟 16）+ `npm run build` 全绿。

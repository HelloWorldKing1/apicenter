...
> 版本：v1.1（2026-09-18）｜ 适用代码基线：M0–M5 + 前置接口编排全部已落地（自动化测试 290 个 @Test）
> 配套文档：[《API中心使用教程》](API中心使用教程.md) ｜ [README](../../../../README.md) ｜ [整体测试方案](开发文档/测试/整体测试方案.md)
...
- **做调用方鉴权、不做授权**（2026-09-23 落地；**2026-09-24 v1.2 模型修订**）：入站鉴权在网关入口执行，
  方式为 API Key / HMAC-SHA256 / Bearer / 仅 IP 名单四种，三态灰度 `client-auth.mode`（`OFF` 默认 → `OPTIONAL` → `ENFORCED`），
  每次判定写 `access_auth_log` 审计（主体 · IP · 方式 · 结果 · **命中的凭证备注/指纹**）。
  **v1.2 的关键变化**：判定中心是「**是否持有平台签发的凭证**」，不是「你是谁」—— 调用方是**未知且持续新增的第三方**，
  所以**不登记也能调**（页面发一把密钥即可）：鉴权方式挂在**接口**上（接口绑定 → 平台默认 → 档案回退），
  凭证放**三级属主凭证池**（接口专属 → 平台共享 → 档案，逐级短路），平台默认与「是否强制自报主体」**页面可改、即时生效**。
  「调用方档案」降级为**可选**的精确管控（需要可验证身份/单独名单/单独配额时才建）。**授权**（能调哪些接口）与临时封禁仍不在本版本；
  入站鉴权**不在接口模型内**（不进接口配置/快照，仅接口级绑定方式）。
...
| `repository/` | JdbcTemplate 数据访问（26 张表，无 JPA） |
...
共 **26 张表**（配置 12 + 运行 8 + 管理面账号 2 + **入站鉴权 4**；其中 interface_step 为前置步骤编排、admin_user/admin_session 为管理面账号登录（2026-09-18）、client_app/client_credential/access_auth_log 为平台入站鉴权（2026-09-23），无数据库外键（引用完整性应用层保证，引用列建索引）。DDL 见 [`schema.sql`](schema.sql)，可视化见 [`表结构设计.html`](表结构设计.html)。
...
**测试基线**：全库 **290 个 @Test**（2026-09-18；含前置编排 `PreStepIntegrationTest` 15、账号登录 `AuthIntegrationTest` 14 + `PasswordHasherTest` 5、HTTP 错误语义 4、读超时 8 等）。

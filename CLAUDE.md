...
| `前置接口编排设计方案.md` | **接口编排（既有接口复用为前置，A → B → 第三方）方案（v0.1.1 评审稿，待拍板，未实施；前置条件 PS-0 已落地）**：OUTBOUND 接口挂 `interface_step` 配置子表，链内 3 处小改（DECODE 可跳过 / MAPPING 前插前置 / ENCODE 剥离保留键 `steps`）+ `PreStepExecutor`（绕开 OutboundEngine，运行期零 DDL）；含失败传播矩阵 / 幂等与 trace / 前端「前置步骤」Tab 界面设计 / 改动面清单（明确不动运行表、不动六阶段枚举）/ D-PS-1~9 待拍板 |
